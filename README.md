...
- Uses an **AI Agent** (powered by DeepSeek via Ollama, or OpenAI as an alternative) to qualify or reject the lead
...
- **AI model** — choose one:
  - [Ollama](https://ollama.com/) running locally with the `deepseek-v3.1:671b-cloud` model *(free, self-hosted)*
  - An [OpenAI API key](https://platform.openai.com/api-keys) *(paid, cloud-based)*
...
4. Make sure the model is pulled and running:
   ```bash
   ollama pull deepseek-v3.1:671b-cloud
   ```

#### Option B: OpenAI (cloud-based alternative)
1. Get your API key from [https://platform.openai.com/api-keys](https://platform.openai.com/api-keys)
2. In n8n, click **"Add credential"** → Search **OpenAI**
3. Paste your API key and save
4. In both workflows, open each **Ollama Chat Model** node and **delete it**
5. Add an **OpenAI Chat Model** node in its place:
   - Click the **+** on the AI Agent node's model input
   - Search for **OpenAI Chat Model**
   - Select your credential and choose a model (e.g., `gpt-4o` or `gpt-4o-mini`)
6. The rest of the workflow stays exactly the same

> **Tip:** `gpt-4o-mini` is the most cost-effective choice for qualification tasks and works well for this use case.
...
| **DeepSeek v3.1 (via Ollama)** | Default AI model for qualification & classification *(self-hosted)* |
| **OpenAI** | Alternative AI model (GPT-4o / GPT-4o-mini) *(cloud-based)* |
...
- If using Ollama, the `deepseek-v3.1:671b-cloud` model must be running and accessible from your n8n instance
- If using OpenAI, replace the **Ollama Chat Model** node in both workflows with an **OpenAI Chat Model** node and connect your API key credential
- Both workflows must be **active** for the full automation to work
- Credentials (Gmail, Ollama/OpenAI) must be reconnected after import — they do not transfer between n8n instances
